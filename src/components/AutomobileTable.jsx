...
const AutomobileTable = () => {
  ...
  const fetchAutomobiles = async () => {
    try {
      ...
      const response = await axios.get('https://backend-tk34.onrender.com/api/automobiles', {
        headers: {
          'x-auth-token': token
        }
      });
  ...
  const handleDelete = async (id) => {
    try {
      const token = localStorage.getItem('token');
      await axios.delete(`https://backend-tk34.onrender.com/api/automobiles/${id}`, {
        headers: {
          'x-auth-token': token
        }
      });
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      ...
      const response = await axios.put(
        `https://backend-tk34.onrender.com/api/automobiles/${currentAutomobile._id}`, 
        currentAutomobile,
        {
          headers: {
            'x-auth-token': token
